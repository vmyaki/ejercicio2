package com.imfe.ejercicio2.clases;
...
public class Planeta {
	...
	public Planeta () {
		...
		/*Elemento tmb se puede hacer as�, con un operador ternario en un misma fila
		 this.elemento=(aleatorio.nextBoolean() ? 'c':'s');
		 Eval�a la expresi�n booleana inicial, y si es true devuelve la primera parte ('c') o la segunda en caso contrario ('s').*/
	...
	//Método si tiene vida, si se cumple el if tieneVida entonces es true sino false
	...
	public Planeta fusion(Planeta planetaFusionar) {
		...
		nuevo.setNombre(this.nombre + "+" + planetaFusionar.getNombre());
		nuevo.setDistancia((this.distancia + planetaFusionar.getDistancia()) / 2d);
		nuevo.setGravedad((this.gravedad + planetaFusionar.getGravedad()) / 2f);
		nuevo.setHayAtmosfera(this.hayAtmosfera || planetaFusionar.isHayAtmosfera());
		if (this.elemento=='c' && planetaFusionar.getElemento()=='c')
		{
			nuevo.setElemento('c');
		}
		else if (this.elemento=='s' && planetaFusionar.getElemento()=='s')
		{
			nuevo.setElemento('s');
		}
		else {
